...
- OpenClaw model speed is configured through OpenClaw itself; the integration test tries the repeatedly verified `ollama/qwen3:8b` first, then falls back to `ollama/llama3.2:latest` and `ollama/qwen2.5-coder:1.5b`, records candidates in evidence, and can be pinned with `RIIDO_OPENCLAW_INTEGRATION_MODEL` before QA.
