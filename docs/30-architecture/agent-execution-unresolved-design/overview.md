...
| C7 security 는 daemon-local auto-approve/fail-closed gate 를 갖고 있으나 headless web approval handoff 는 아직 runtime lifecycle 과 연결되지 않았다. | `docs/20-domain/security.md` §6, `internal/agentbridge/toolpolicy`, `internal/agentbridge/session` | provider approval RPC 가 있으면 policy-bundle allowlist 만 자동 승인하고, approval round-trip 없이 관측된 classified tool start 는 fail-closed 한다. web approval timeout/decision DTO 는 아직 shared lifecycle 로 승격되지 않았다. |
...
| F6 | headless tool approvals blocked | **Partially resolved for daemon-local risk gates.** policy bundle 에 명시된 classified surface 만 auto-approve 하고, approval round-trip 없이 시작된 destructive/protected/network/secret surface 는 provider 를 kill 하고 `ResultBlocked` 로 수렴한다. SaaS/web approval request/decision round-trip 은 아직 없음 | web approval request/decision DTO 와 timeout terminal state 를 contracts/control-plane lifecycle 로 승격 |
...
| R4 | daemon restart 후 처음부터 반복 | **Partially resolved for pinned provider sessions.** control-plane 이 assignment 에 저장한 `provider_session_id` 를 daemon 이 active payload 재구성 시 `resume_session_id` 보다 우선 사용한다. run attempt / `recovery_mode` durable table 은 아직 분리되지 않았다. | provider session pin/resume 회귀 테스트 유지; 후속으로 assignment run table 에 `attempt`, `recovery_mode` 기록 |
| R5 | cancellation watcher leak | **Resolved for daemon supervisor/saasplane.** `saasplane` closes execution-id keyed watcher channels on terminal paths, and supervisor cancels a per-task lifetime context on complete/cancel/shutdown so source implementations that leave watcher channels open cannot leak goroutines. | terminal close/release invariant 회귀 테스트 유지; contracts/FSM 승격 시 동일 규칙을 conformance 로 고정 |
