...
The daemon reports unavailable providers instead of executing a different binary than the operator selected. Runtime start must reuse the executable selected by Detect and must not re-resolve a same-name binary from `PATH`.

OpenClaw may probe later same-name PATH candidates only when `RIIDO_OPENCLAW_PATH` is unset. An explicit OpenClaw override is a pin and never falls back to PATH.
