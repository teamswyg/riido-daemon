...
- loop count: `88`
- registered loop files: `88`
- evidence items: `188`
- open gaps: `0`
- phase coverage: `observe=88/88`, `hypothesis=88/88`, `execute=88/88`, `evaluate=88/88`, `retrospective=88/88`
...
- Owner: `tools/localproductacceptance and local-qa-runner workflow`
- Observe: Local QA added many harness and generated evidence surfaces, but meta complexity, product outcome linkage, and partial reduction debt were not measured as one loop.
  - Artifacts: `docs/30-architecture/closed-loop-maturity.dsl.json`
- Hypothesis: A compact maturity DSL can make harness growth, product outcome linkage, and stale partial candidates visible without changing runtime contracts.
  - Artifacts: `tools/localproductacceptance/closed_loop_maturity_test.go`
- Execute: Generate the maturity DSL mirror, run localproductacceptance, and publish closed-loop maturity fields into product acceptance evidence.
  - Artifacts: `tools/localproductacceptance`, `.github/workflows/local-qa-runner.yml`
- Evaluate: The verifier rejects generated DSL drift and CI greps for local.qa.closed_loop_maturity, meta_complexity, product_acceptance, and partial_reduction evidence.
  - Artifacts: `tools/localproductacceptance/closed_loop_maturity_test.go`, `.github/workflows/local-qa-runner.yml`
- Retrospective: New harnesses now have a visible maturity row; inferred product and partial metrics stay partial until promoted into runner-backed verifiers.
  - Artifacts: `docs/30-architecture/loop-engineering.md`
- Evidence:
  - `command`: go test ./tools/localproductacceptance && go run ./tools/localproductacceptance -client-root /tmp/riido-daemon-loop-client -probe-routes=false -storage-state /tmp/no-riido-storage-state.json -workspace-id "" -api-token "" -evidence-out /tmp/ai-agent-product-acceptance.json -lab-out /tmp/contract-lab/index.html -manual-evidence-out /tmp/manual-qa-evidence.json; proves closed-loop maturity DSL is fresh and maturity evidence is emitted
  - `workflow`: .github/workflows/local-qa-runner.yml; proves public CI preserves closed-loop maturity evidence fields
...
### local-qa-closed-loop-candidates
...
### local-qa-daily-trigger

- Owner: `tools/localqaschedule, localqarunner, and local QA dashboard`
- Observe: Local QA evidence had daily scheduling and expires_at wording, but the dashboard and semantic binding could not prove that expired evidence is refreshed instead of merely displayed as stale.
  - Artifacts: `docs/30-architecture/local-qa-daily-trigger.dsl.json`
- Hypothesis: A schedule evidence field for expired-evidence refresh, plus dashboard coverage and semantic binding peers, can make the daily QA loop fail when freshness ownership drifts.
  - Artifacts: `tools/localqaschedule/evidence_test.go`, `tools/localqadashboard/schedule_evidence_freshness_test.go`
- Execute: Emit refreshes_expired_evidence in schedule evidence, read freshness fields in the dashboard, and expose local.qa.daily_freshness as a coverage scenario.
  - Artifacts: `tools/localqaschedule/trigger.go`, `tools/localqadashboard/schedule_evidence.go`, `docs/30-architecture/local-acceptance-coverage.riido.json`
- Evaluate: Unit tests and local-qa-runner CI verify the generated schedule evidence, freshness failure mode, coverage row, and workflow grep evidence.
  - Artifacts: `tools/localqadashboard/schedule_evidence_freshness_test.go`, `.github/workflows/local-qa-runner.yml`
- Retrospective: Expired evidence now has an owner loop: daily schedule evidence must prove it refreshes stale rows, and semantic binding blocks one-sided changes to that claim.
  - Artifacts: `docs/30-architecture/loop-engineering.md`
- Evidence:
  - `command`: go test ./tools/localqaschedule ./tools/localqadashboard -count=1; proves schedule evidence and dashboard freshness coverage are tested together
  - `workflow`: .github/workflows/local-qa-runner.yml; proves public CI preserves refreshes_expired_evidence and local.qa.daily_freshness artifacts
