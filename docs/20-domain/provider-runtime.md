...
- `internal/agentbridge/session`, `runtimeactor`, `supervisor`, `bridge`, `controlplane`, `detectutil`
...
후속 session/runtimeactor/provider-adapter migration slice 가 맡는다.
...
본 문서는 시그니처의 **도메인 표현** 을 박는다. public Go boundary 의 현재 구현은 [`internal/agentbridge`](../../internal/agentbridge) 루트 package 와 [`docs/migration/daemon.md`](../migration/daemon.md) 의 RIID-4651 slice 가 확정한다. session/runtimeactor/supervisor package boundary 는 후속 migration slice 에서 같은 문서를 갱신해야 한다.
...
4. `internal/agentbridge/session` 이 public repo 로 이동하는 slice 는 본 문서, session 상수, `TestSessionDefaultBuffersMatchProviderRuntimeBackpressureSSOT`, 그리고 provider-runtime-backpressure workflow 를 같은 work unit 에서 갱신해야 한다.
