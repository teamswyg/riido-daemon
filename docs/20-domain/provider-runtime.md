...
- remaining concrete provider adapter (`cursor`)
...
본 문서는 시그니처의 **도메인 표현** 을 박는다. public Go boundary 의 현재 구현은 [`internal/agentbridge`](../../internal/agentbridge) 루트 package, [`internal/agentbridge/session`](../../internal/agentbridge/session), [`internal/agentbridge/bridge`](../../internal/agentbridge/bridge), [`internal/agentbridge/detectutil`](../../internal/agentbridge/detectutil), [`internal/agentbridge/runtimeactor`](../../internal/agentbridge/runtimeactor), [`internal/agentbridge/controlplane`](../../internal/agentbridge/controlplane), [`internal/provider/claude`](../../internal/provider/claude), [`internal/provider/codex`](../../internal/provider/codex), [`internal/provider/openclaw`](../../internal/provider/openclaw), 그리고 [`docs/migration/daemon.md`](../migration/daemon.md) 의 RIID-4651 / RIID-4653 / RIID-4654 / RIID-4656 / RIID-4657 / RIID-4658 / RIID-4659 / RIID-4660 slice 가 확정한다. supervisor package boundary 와 나머지 concrete provider adapter boundary 는 후속 migration slice 에서 같은 문서를 갱신해야 한다.
...
| compatibility gate architecture SSOT (후속 architecture-doc migration slice) | G5 (Pre-Execute) 핸드셰이크는 adapter 측 `initialize` / `firstline` probe 를 호출. 그 결과로 lease 가 활성화. |
| runtime upgrade flow architecture SSOT (후속 architecture-doc migration slice) | adapter 가 `Running` 도중 `RuntimeID`/`CapabilityFingerprint` 변경을 감지하면 즉시 draft `Type=RuntimePinViolated` 를 발행하고 process 를 stop. |
...
## 10. 어댑터 4 종 — 본 컨텍스트 안에서의 위치

각 어댑터의 capability·protocol·우선순위는 public `riido-contracts/docs/20-domain/provider-capability.md` §4 가 소유한다. 본 문서는 **그 4 종이 본 컨텍스트에서 어떻게 표현되는지** 만 적는다.
...
[`internal/provider/openclaw`](../../internal/provider/openclaw) 로 이동했다. 나머지
adapter 구현은 후속 migration slice 에서 이동한다.
...
| `CodexExecJSONLAdapter` | `codex exec --json` 의 NDJSON 이벤트를 흡수. `turn.started` / `item.*` 명명 (dot 형식). MVP stable path. | Cat C 위주 |
| `CodexAppServerAdapter` | `codex app-server --listen stdio://` JSON-RPC. `initialize` 핸드셰이크로 `Capability` probe. `turn/started` / `item/*` 명명 (slash 형식). experimental — opt-in 필요. | Cat C + approval (Cat C `ApprovalRequested`) |
| `ClaudeCompatibleWrapperAdapter` | wrapper 매니페스트의 자기 신고에 따라 동작. `trustTier` 가 capability 에 반영되어 ingest 계층이 정책 게이트를 추가 적용. | 자기 신고된 surface 의 교집합 |
...
Open questions roadmap 문서는 후속 architecture-doc migration slice 에서 public repo 로 이동한다. 그 전까지는 이 섹션과 Riido task 가 C4 runtime 미결정의 public-facing 위치다. `Q-RT-001` 은 §7.5 로 닫혔고, `Q-RT-005` 는 §8 로 닫혔다.
