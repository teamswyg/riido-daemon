...
C4 Provider Runtime owns provider process stream, provider draft/session event channel, actor mailbox sizes, and drop policy. C6 workspace, C7 policy, and C10 server do not redefine these values.
...
6. changing buffer/mailbox values requires one work unit to update this SSOT, implementation constants, default-size tests, and CI evidence.
