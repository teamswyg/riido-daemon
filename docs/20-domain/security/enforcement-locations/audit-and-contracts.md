...
- **모든 게이트 실패는 IR 이벤트로 남는다.** 운영자가 “왜 거절되었는지” 를 항상 replay 할 수 있다.
- **scoped token 의 값** 은 IR 에 적지 않는다. `scopeID`, `ttlSeconds`, `purpose` 만.
- **secret 패턴** 이 IR payload 에 들어갈 위험이 있는 경우, adapter ACL 단계에서 redact 한다(`Unknown` 에도 적지 않는다). 정확한 redaction 규칙은 [`../security-redaction.md`](../security-redaction.md).
- **trust tier 결정 결과** 는 `RuntimeRegistered` payload 에 포함되어 영속된다.
