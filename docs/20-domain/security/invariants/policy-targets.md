...
Each target is a member of `AllowedSurfaceSet` and is decided independently by trust tier.
...
Secret exposure redaction marker/catalog details are owned by [`../security-redaction.md`](../security-redaction.md).

This document owns target enum values and meanings. Adjacent contexts consume only decisions, such as "T-SBX -> workspace-write".
