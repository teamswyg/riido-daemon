...
| T-SBX | sandbox mode | enum + provider 별 activation 결정 (`read-only` / `workspace-write` / policy-owned `danger-full-access`; Codex current full-access envelope 는 §4.2 처럼 C4-owned) |
...
`codex app-server --sandbox danger-full-access --listen stdio://` 를 daemon-owned
...
### 4.2 Provider full-access runtime harness
...
`--sandbox`, `--sandbox=*`, `-c`, `--config`, `--enable`, `--disable`, `--yolo`,
`--dangerously-bypass-approvals-and-sandbox` 를 drop 하고 `DroppedArgs` 로 남긴다.
...
Codex 의 approval-bypass 표면은 현재 local daemon command builder 가 생성하지 않으며, free-form `CustomArgs` 에서도 차단된다. Codex `--sandbox danger-full-access` 는 §4.2 의 provider full-access runtime envelope 로서 C4 command builder 가 직접 생성하며, §5 unsafe bypass policy bundle surface 로 다루지 않는다. 추후 provider-native approval bypass 표면을 의도적으로 허용하는 PR 은 `StartOptions` 같은 명시적 입력과 `internal/policy.EvaluateUnsafeBypass` 호출을 먼저 추가해야 하며, Host / Unknown trust tier 는 여전히 항상 거절한다.
