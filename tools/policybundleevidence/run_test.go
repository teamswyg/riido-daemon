...
func TestCurrentManifest(t *testing.T) {
	if err := run(options{Repo: "../..", Manifest: "docs/20-domain/security/invariants/policy-bundle-loader.riido.json"}); err != nil {
		t.Fatal(err)
	}
}

func TestGeneratedDocCurrent(t *testing.T) {
	opts := options{Repo: "../..", Manifest: "docs/20-domain/security/invariants/policy-bundle-loader.riido.json", CheckDoc: true}
