...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustResponsibilityWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertResponsibilityProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustResponsibilityWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func mustResponsibilityWrite(t *testing.T, path, body string) {
	t.Helper()
