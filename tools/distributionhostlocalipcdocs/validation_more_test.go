...
func TestValidateBlocksReportsInvalidShapes(t *testing.T) {
	...
	for _, want := range []string{
		...
		assertDistributionHostLocalIPCProblem(t, problems, want)
	}
}
...
func TestValidateDetailsReportsSchemaRequiredAndDuplicateIssues(t *testing.T) {
	...
	assertDistributionHostLocalIPCProblem(t, problems, "unexpected detail schema_version")
	assertDistributionHostLocalIPCProblem(t, problems, "detail id, title, generated_doc, and blocks are required")
	assertDistributionHostLocalIPCProblem(t, problems, "duplicate detail id same")
}

func assertDistributionHostLocalIPCProblem(t *testing.T, problems []string, needle string) {
	t.Helper()
