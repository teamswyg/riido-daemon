...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustDistributionHostLocalIPCWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertDistributionHostLocalIPCProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustDistributionHostLocalIPCWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func TestSourceCheckAndPathsFailures(t *testing.T) {
	...
	mustDistributionHostLocalIPCWrite(t, target, "anchor")
	...
	assertDistributionHostLocalIPCProblem(t, mustExist(dir, "missing.yml"), "missing artifact")
}

func mustDistributionHostLocalIPCWrite(t *testing.T, path, body string) {
	t.Helper()
