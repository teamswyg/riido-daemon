...
const (
	manifestSchema = "riido-provider-integration-gate-docs.v1"
	detailSchema   = "riido-provider-integration-gate-detail.v1"
)
...
func validateManifest(repo string, m manifest) ([]string, []sourceCheckResult) {
	...
	if len(m.DetailPages) != 9 || len(m.Details) != 9 {
		problems = append(problems, "nine generated detail pages are required")
	}
