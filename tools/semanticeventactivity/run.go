...
func run(_ context.Context, opts options) error {
	...
	problems := validate(opts.Repo, manifest)
	...
	if opts.EvidenceOut != "" {
		if err := writeJSON(opts.EvidenceOut, buildEvidence(manifest, problems)); err != nil {
			return err
		}
	}
	if len(problems) > 0 {
	...
	fmt.Println("semantic-event-activity: clean")
	return nil
}
...
func checkDoc(opts options, manifest Manifest, rendered string) []problem {
	...
	if string(actual) != rendered {
		return []problem{{"generated doc drift: run go run ./tools/semanticeventactivity -write-doc"}}
	}
	return nil
}
