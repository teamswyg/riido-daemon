...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustWorkspaceNativeWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertWorkspaceNativeProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustWorkspaceNativeWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func mustWorkspaceNativeWrite(t *testing.T, path, body string) {
	t.Helper()
