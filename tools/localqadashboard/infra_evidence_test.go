...
func TestInfraEvidenceScenariosPassesOfficeIPRestrictedS3Dashboard(t *testing.T) {
	path := filepath.Join(t.TempDir(), "infra.json")
	body := `{"terraform_managed":true,"public_access_blocked":false,"public_acls_blocked":true,"read_access_mode":"source_ip_allowlist","read_source_cidrs":["121.157.229.25/32"],"bucket_policy_is_public":false,"bucket_policy_source_ip_restricted":true,"encryption_algorithm":"AES256","lifecycle_expire_days":30,"latest_index_observed":true,"latest_index_bytes":100,"latest_cache_control":"no-store","latest_object_sse":"AES256"}`
	if err := os.WriteFile(path, []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
	got := infraEvidenceScenarios(path)
