...
{{if .Run.ClosedLoops}}<h2>Closed-loop Candidates</h2><table><thead><tr><th>Candidate</th><th>Class</th><th>Observation</th><th>Next Loop</th></tr></thead><tbody>{{range .Run.ClosedLoops}}<tr><td><code>{{.ID}}</code>{{if .SourceScenario}}<br>{{.SourceScenario}}{{end}}</td><td>{{.Class}}</td><td>{{.Reason}}<br><code>{{.NextEvidence}}</code></td><td><code>{{.Graph.NextLoop}}</code></td></tr>{{end}}</tbody></table>{{end}}
<h2>Coverage</h2>
