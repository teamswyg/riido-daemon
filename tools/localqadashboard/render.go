...
{{if .Run.Candidates}}<h2>Closed-Loop Candidates</h2><table><thead><tr><th>ID</th><th>Source</th><th>Trigger</th><th>Status</th><th>Age</th><th>Evidence</th><th>Decision</th><th>Next action</th></tr></thead><tbody>{{range .Run.Candidates}}<tr><td><code>{{.ID}}</code></td><td>{{.Source}}</td><td>{{.Trigger}}</td><td class="status">{{.Status}}</td><td>{{.AgeHours}}h{{if .StaleAt}}<br>stale {{.StaleAt}}{{end}}</td><td>{{if .Evidence}}<code>{{.Evidence}}</code>{{end}}</td><td>{{.Graph.Decision}}{{if .Graph.NextLoop}}<br><code>{{.Graph.NextLoop}}</code>{{end}}</td><td>{{.Summary}}<br>{{.NextAction}}</td></tr>{{end}}</tbody></table>{{end}}
{{if .Run.ClosedLoops}}<h2>Product Closed-loop Candidates</h2><table><thead><tr><th>Candidate</th><th>Class</th><th>Observation</th><th>Required Artifacts</th><th>Next Loop</th></tr></thead><tbody>{{range .Run.ClosedLoops}}<tr><td><code>{{.ID}}</code>{{if .SourceScenario}}<br>{{.SourceScenario}}{{end}}</td><td>{{.Class}}</td><td>{{.Reason}}<br><code>{{.NextEvidence}}</code></td><td>{{range .RequiredNextArtifacts}}<code>{{.}}</code><br>{{end}}</td><td><code>{{.Graph.NextLoop}}</code></td></tr>{{end}}</tbody></table>{{end}}
<h2>Coverage</h2>
