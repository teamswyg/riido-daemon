...
func TestScheduleEvidenceScenariosAcceptsActiveLaunchdRun(t *testing.T) {
	...
	body := `{"status":"passed","installed":true,"s3_prefix_configured":true,"coverage_evidence":"coverage.json","launchd":{"checked":true,"loaded":true,"state":"running","calendar_trigger":true,"runs":1,"last_exit_code":"(never exited)"}}`
