...
const (
	manifestSchema = "riido-locking-docs.v1"
	detailSchema   = "riido-locking-detail.v1"
)
...
func validateManifest(repo string, m manifest) ([]string, []sourceCheckResult) {
	...
	if len(m.DetailPages) != len(m.Details) || len(m.Invariants) == 0 {
		problems = append(problems, "detail pages and invariants are required")
	}
