...
func run(opts options) error {
	...
	problems, sources, gates, absent := validate(opts.Repo, manifest)
	...
	if opts.EvidenceOut != "" {
		ev := buildEvidence(manifest, problems, sources, gates, absent)
	...
	fmt.Println("runtime-eligibility-evidence: clean")
	return nil
}
...
func checkDoc(opts options, rel, body string) []problem {
	...
	if string(current) != body {
		return []problem{{Message: "generated doc drift: run tools/runtimeeligibility -write-doc"}}
	}
	return nil
}
