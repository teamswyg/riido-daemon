...
func TestValidateBlocksReportsInvalidShapes(t *testing.T) {
	...
	for _, want := range []string{
		...
		assertDistributionHostInvariantsProblem(t, problems, want)
	}
}
...
func TestValidateDetailsReportsSchemaRequiredAndDuplicateIssues(t *testing.T) {
	...
	assertDistributionHostInvariantsProblem(t, problems, "unexpected detail schema_version")
	assertDistributionHostInvariantsProblem(t, problems, "detail id, title, generated_doc, and blocks are required")
	assertDistributionHostInvariantsProblem(t, problems, "duplicate detail id same")
}

func assertDistributionHostInvariantsProblem(t *testing.T, problems []string, needle string) {
	t.Helper()
