...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustDistributionHostInvariantsWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertDistributionHostInvariantsProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustDistributionHostInvariantsWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func TestSourceCheckAndPathsFailures(t *testing.T) {
	...
	mustDistributionHostInvariantsWrite(t, target, "anchor")
	...
	assertDistributionHostInvariantsProblem(t, mustExist(dir, "missing.yml"), "missing artifact")
}

func mustDistributionHostInvariantsWrite(t *testing.T, path, body string) {
	t.Helper()
