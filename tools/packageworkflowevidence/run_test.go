...
func TestRunLoadsWorkflowSources(t *testing.T) {
	dir := t.TempDir()
	manifestPath := filepath.Join(dir, "manifest.json")
	sourcePath := filepath.Join(dir, "sources", "example.json")
	workflowPath := filepath.Join(dir, ".github", "workflows", "example.yml")
	mustWrite(t, manifestPath, `{"schema_version":"riido-daemon-package-workflow-evidence.v1","id":"m","loop_source":"loops/m.json","workflow_sources":["sources/example.json"],"workflows":[]}`)
	mustWrite(t, sourcePath, `{"schema_version":"riido-daemon-package-workflow-evidence.v1","id":"fragment","workflows":[{"id":"example","workflow":"`+filepath.ToSlash(workflowPath)+`","evidence_artifact":"example","required_fragments":["go test ./example"]}]}`)
	mustWrite(t, workflowPath, "run: go test ./example\n")
	out := filepath.Join(dir, "out", "evidence.json")
	if err := run([]string{"-manifest", manifestPath, "-workflow", workflowPath, "-evidence-out", out}); err != nil {
		t.Fatal(err)
