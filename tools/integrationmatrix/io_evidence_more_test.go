...
func TestLoadJSONRejectsUnknownAndTrailingValues(t *testing.T) {
	dir := t.TempDir()
	valid := filepath.Join(dir, "valid.json")
	if err := os.WriteFile(valid, []byte(`{"schema_version":"v","id":"id"}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if _, err := loadJSON[manifest](valid); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := loadJSON[manifest](unknown); err == nil {
	...
	if _, err := loadJSON[manifest](trailing); err == nil {
