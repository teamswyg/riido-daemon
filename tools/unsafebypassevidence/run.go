...
func run(opts options) error {
	...
	problems, sourceEvidence, policyEvidence, codexEvidence := validate(opts.Repo, manifest)
	...
	if opts.EvidenceOut != "" {
		ev := buildEvidence(manifest, problems, sourceEvidence, policyEvidence, codexEvidence)
	...
	fmt.Println("unsafe-bypass-evidence: clean")
	return nil
}
...
func checkDoc(opts options, rel, body string) []problem {
	...
	if string(current) != body {
		return []problem{{Message: "generated doc drift: run tools/unsafebypassevidence -write-doc"}}
	}
	return nil
}
