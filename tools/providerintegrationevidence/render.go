...
func renderMarkdown(m manifest) string {
	...
	b.WriteString("- OpenClaw model speed is configured through OpenClaw itself; the integration test tries `ollama/qwen2.5-coder:1.5b` first, falls back to `ollama/llama3.2:latest` and `ollama/qwen3:8b`, records candidates in evidence, and can be pinned with `RIIDO_OPENCLAW_INTEGRATION_MODEL` before QA.\n")
