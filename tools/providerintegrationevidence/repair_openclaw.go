...
func openClawModelConfigRepair() repair {
	return repair{
		...
		SuggestedCommand: "openclaw doctor --fix && openclaw models set ollama/llama3.2:latest && RIIDO_OPENCLAW_INTEGRATION_MODEL=ollama/llama3.2:latest AGENTBRIDGE_INTEGRATION=1 go test ./internal/provider/openclaw -race -count=1 -run TestIntegration -v",
	}
}
