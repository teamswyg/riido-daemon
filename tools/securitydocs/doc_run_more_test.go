...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustSecurityDocsWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	assertSecurityDocsProblem(t, checkDocs(options{Repo: dir, CheckDoc: true}, docs), "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustSecurityDocsWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func TestSourceCheckAndPathsFailures(t *testing.T) {
	...
	mustSecurityDocsWrite(t, target, "anchor")
	...
	assertSecurityDocsProblem(t, mustExist(dir, "missing.yml"), "missing artifact")
}

func mustSecurityDocsWrite(t *testing.T, path, body string) {
	t.Helper()
