...
const (
	manifestSchema = "riido-distribution-host-docs.v1"
	pageSchema     = "riido-distribution-host-page.v1"
)
...
func validateManifest(repo string, m manifest) ([]string, []sourceCheckResult) {
	...
	if len(m.Parts) != 3 || len(m.RelatedPages) == 0 || len(m.Assertions) == 0 {
		problems = append(problems, "parts, related pages, and assertions are required")
	}
...
func validatePages(pages []page) []string {
	if len(pages) != 4 {
		return []string{"four generated distribution host pages are required"}
	}
	var problems []string
