...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustBoundaryWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertBoundaryProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustBoundaryWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func mustBoundaryWrite(t *testing.T, path, body string) {
	t.Helper()
