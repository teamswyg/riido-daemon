...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	if problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs); len(problems) != 0 {
		t.Fatalf("expected clean docs, got %#v", problems)
	}
	mustWorkspaceInvariantWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertWorkspaceInvariantProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustWorkspaceInvariantWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func mustWorkspaceInvariantWrite(t *testing.T, path, body string) {
	t.Helper()
