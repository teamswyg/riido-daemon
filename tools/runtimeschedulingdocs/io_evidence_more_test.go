...
func TestBuildEvidenceAndProblemStatus(t *testing.T) {
	m := manifest{
		...
		Core: coreDoc{
			GeneratedDoc: "core.md",
			Invariants:   []invariant{{Name: "one"}},
		},
	}
	checks := []sourceCheckResult{{Name: "source", File: "file", Passed: true}}
	ok := buildEvidence(m, checks, nil)
	if ok.Status != "verified" || ok.InvariantCount != 1 || ok.AssertionCount != 2 {
...
func TestReadJSONRejectsUnknownAndTrailingValues(t *testing.T) {
	...
	var core coreDoc
	if err := readJSON(valid, &core); err != nil {
		t.Fatal(err)
	}
	...
	if err := readJSON(unknown, &core); err == nil {
	...
	if err := readJSON(trailing, &core); err == nil {
