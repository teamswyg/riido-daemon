...
func TestValidateBlocksReportsInvalidShapes(t *testing.T) {
	...
	for _, want := range []string{
		...
		assertOverlayProblem(t, problems, want)
	}
}
...
func TestValidateDetailsReportsSchemaRequiredAndDuplicateIssues(t *testing.T) {
	...
	assertOverlayProblem(t, problems, "unexpected detail schema_version")
	assertOverlayProblem(t, problems, "detail id, title, generated_doc, and blocks are required")
	assertOverlayProblem(t, problems, "duplicate detail id same")
}

func TestRenderBlockCoversCode(t *testing.T) {
	var b strings.Builder
	renderBlock(&b, block{Kind: "code", Language: "go", Code: "fmt.Println()"})
	if out := b.String(); !strings.Contains(out, "```go\nfmt.Println()") {
		t.Fatalf("rendered block = %q", out)
	}
}

func assertOverlayProblem(t *testing.T, problems []string, needle string) {
	t.Helper()
