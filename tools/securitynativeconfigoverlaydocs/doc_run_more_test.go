...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustOverlayWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertOverlayProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustOverlayWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func mustOverlayWrite(t *testing.T, path, body string) {
	t.Helper()
