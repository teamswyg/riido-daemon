...
const (
	manifestSchema = "riido-security-native-config-overlay-docs.v1"
	detailSchema   = "riido-security-native-config-overlay-detail.v1"
)
...
func validateManifest(repo string, m manifest) ([]string, []sourceCheckResult) {
	...
	if len(m.DetailPages) != 6 || len(m.Details) != 6 {
		problems = append(problems, "six generated detail pages are required")
	}
