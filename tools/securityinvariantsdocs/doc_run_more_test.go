...
func TestMaybeWriteAndCheckDocs(t *testing.T) {
	...
	mustSecurityInvariantWrite(t, filepath.Join(dir, "docs/out.md"), "stale")
	problems := checkDocs(options{Repo: dir, CheckDoc: true}, docs)
	assertSecurityInvariantProblem(t, problems, "generated doc drift")
}

func TestRunWritesFailedEvidenceForInvalidManifest(t *testing.T) {
	dir := t.TempDir()
	mustSecurityInvariantWrite(t, filepath.Join(dir, "manifest.json"), `{
  "schema_version": "bad",
  "id": "x",
...
func mustSecurityInvariantWrite(t *testing.T, path, body string) {
	t.Helper()
