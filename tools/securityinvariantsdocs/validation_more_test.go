...
func TestValidateBlocksReportsInvalidShapes(t *testing.T) {
	...
	for _, want := range []string{
		...
		assertSecurityInvariantProblem(t, problems, want)
	}
}
...
func TestValidateDetailsReportsSchemaRequiredAndDuplicateIssues(t *testing.T) {
	...
	assertSecurityInvariantProblem(t, problems, "unexpected detail schema_version")
	assertSecurityInvariantProblem(t, problems, "detail id, title, generated_doc, and blocks are required")
	assertSecurityInvariantProblem(t, problems, "duplicate detail id same")
}

func assertSecurityInvariantProblem(t *testing.T, problems []string, needle string) {
	t.Helper()
