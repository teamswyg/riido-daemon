...
- `riido_daemon_private` 또는 `riido-daemon-private` repository를 참고하거나 수정하지 않습니다. 누락된 사실은 이 public repo의 SSOT에 새로 정의하거나 `riido-contracts`로 승격합니다.
...
| Riido 작업 `branchName` 사용 강제 | [`docs/30-architecture/riido-work-branch-gate.md`](docs/30-architecture/riido-work-branch-gate.md) |
| App Store/MSIX/Developer ID 배포 기준 | [`docs/30-architecture/store-distribution.md`](docs/30-architecture/store-distribution.md) |
| GitHub Release binary와 curl 설치 경로 | [`docs/30-architecture/release-artifacts.md`](docs/30-architecture/release-artifacts.md) |
...
| public daemon migration 이력과 retired private source 금지 | [`docs/migration/daemon.md`](docs/migration/daemon.md) |
...
Desktop/MSIX launcher 는 GitHub Release asset 을 app user data 영역에 다운로드한
뒤 실행합니다. 자세한 배포 경계는
[`docs/30-architecture/release-artifacts.md`](docs/30-architecture/release-artifacts.md)
를 봅니다.
